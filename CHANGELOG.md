...
- Tidying: dropped the URL-hygiene and newsletter-specific sections
  from `references/formatting.md`, plus the URL-hygiene pointers in
  `SKILL.md`, `README.md`, `styles/cote/general.md`,
  `styles/cote/social-media.md`, and
  `references/ai-writing-tells.md`. Both encoded one person's
  publishing setup rather than portable style guidance; newsletter
  house style belongs in a newsletter tool, not a style guide. Fixed
  typos and stale `SKILL.md` cross-references in
  `styles/cote/casual-voice.md` (the ban list and voice summary moved
  to `styles/cote/general.md` in 1.2). Refreshed the `dist/`
  artifacts, which had not been rebuilt since 1.4.
